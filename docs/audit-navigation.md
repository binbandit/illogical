...
## Session rename follow-up, 21 September 2026

The session picker now offers a pencil beside every session and a Rename Session context menu. Sidebar renaming uses the same captured target without switching the active terminal. The sheet retains its original host/session/tab if focus changes elsewhere, trims surrounding whitespace, disables blank submissions and reports a target removed while the sheet was open.

Native QA renamed the inactive `silver-tide` session to `Work QA`, while the header remained on `quiet-cedar`. The picker immediately showed the new name. Whitespace disabled the Rename button; cancellation preserved the saved name, and terminal typing worked without an extra click. `scripts/test-workspace-rename.sh` verifies exact socket targets across host/session/tab selection changes and rejects canceled/deleted targets. The service regression and a real disposable CLI workflow verify watcher broadcasts, durable names after a service restart, and unchanged live process/layout during the rename. Restarting the service still creates new shells by design; rename itself never restarts them.
